...
from sklearn.preprocessing import LabelEncoder # Mungkin dibutuhkan jika ingin mengembalikan label kategorikal
...
def load_data():
    ...
    try:
        ...
        # Contoh: Jika DayOfWeek adalah numerik (0=Senin, ..., 6=Minggu)
...
if option == "Beranda":
    ...
    st.image("https://upload.wikimedia.org/wikipedia/commons/thumb/e/e4/Traffic_accident_on_a_highway_in_Italy.jpg/1200px-Traffic_accident_on_a_highway_in_Italy.jpg", use_column_width=True, caption="Ilustrasi Kecelakaan Lalu Lintas")
    st.markdown("""
    ...
    # Saya akan menggunakan nama kolom yang Anda berikan:
    ...
    try:
        ...
        st.warning("Kolom 'Weather_Condition_Encoded' tidak ditemukan di data mentah.")
    except Exception as e:
    ...
    try:
        ...
        st.warning("Kolom 'Vehicles_Involved' tidak ditemukan di data mentah.")
    except Exception as e:
    ...
    try:
        ...
        st.warning("Kolom 'DayOfWeek' atau 'NamaHari' tidak ditemukan di data mentah.")
    except Exception as e:
    ...
    try:
        ...
        ax4.tick_params(axis='x', rotation=60, ha='right')
        plt.tight_layout()
        st.pyplot(fig4)
    except KeyError:
        st.warning("Kolom 'Location_Encoded' atau 'Severity_Encoded' tidak ditemukan di data mentah.")
    except Exception as e:
    ...
    try:
        if not df_scaled.empty and df_scaled.shape[0] >= 2:
            ...
            for i in range(1, max_clusters_allowed):
                # n_init='auto' atau n_init=10 adalah pilihan yang baik untuk menghindari peringatan
    ...
    try:
        ...
        ax7.tick_params(axis='x', rotation=45, ha='right')
        plt.tight_layout()
        st.pyplot(fig7)
    except KeyError:
        st.warning("Kolom 'Weather_Condition_Encoded' atau 'Severity_Encoded' tidak ditemukan di data mentah.")
    except Exception as e:
